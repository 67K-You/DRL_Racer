...
#        IN ORDER TO MODIFY THE GENERATED GLOBAL SHAPE OF THE ROAD          #
#############################################################################

SCALE = 6.0  # Track scale
TRACK_RAD = 2500 / SCALE  # Track is heavily morphed circle with this radius
PLAYFIELD = 5000 / SCALE  # Game over boundary
FPS = 50  # Frames per second
ZOOM = 0.7  # Camera zoom
ZOOM_FOLLOW = False  # Set to False for fixed view (don't use zoom)

TRACK_DETAIL_STEP = 15 / SCALE
TRACK_TURN_RATE = 0.05
    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            if not tile.road_visited:
                ...
                self.env.reward += 2500.0 / len(self.env.track)
...
class CarRacing(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        self.car.hull.position = 569.5, 61.5
        self.car.hull.angle = 90.03225
    ...
    def step(self, action):
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            if self.tile_visited_count == 175:
                done = True
            ...
            #############################################################################
            #                    ATTEMPT TO IMPLEMENT A HITBOX                          #
            #   THAT PREVENT STOP THE EPISODE IF THE CAR DRIVES TOO MUCH ON THE GRASS   #
            #                  ---------------------------------                        #
            #                               ABORTED                                     #
            #############################################################################
            
        #     if self.step_index > 50:
        #         # Top-Left
        #         if not np.allclose(np.asarray(self.state[66, 45]), np.asarray([102, 102, 102]), atol=10):
        #             # print("1", np.asarray(self.state[66, 45]))
        #             self.counter_grass += 1
        #             step_reward = -10
        #         # Bottom-Left
        #         elif not np.allclose(np.asarray(self.state[77, 45]), np.asarray([102, 102, 102]), atol=10):
        #             # print("2", np.asarray(self.state[77, 45]))
        #             self.counter_grass += 0.5
        #             step_reward = -5
        #         # Top-Right
        #         elif not np.allclose(np.asarray(self.state[66, 50]), np.asarray([102, 102, 102]), atol=10):
        #             # print("3", np.asarray(self.state[66, 50]))
        #             self.counter_grass += 1
        #             step_reward = -10
        #         # Bottom-Right
        #         elif not np.allclose(np.asarray(self.state[77, 50]), np.asarray([102, 102, 102]), atol=10):
        #             # print("4", np.asarray(self.state[77, 50]))
        #             self.counter_grass += 0.5
        #             step_reward = -5
        #         else:
        #             self.counter_grass = 0
                    
        #     car_vel = np.linalg.norm((self.car.hull.linearVelocity))
        #     if car_vel != 0:
        #         if car_vel < 20:
        #             coef = 0.03
        #         elif car_vel >= 20 and car_vel < 50:
        #             coef = 0.05
        #         else:
        #             coef = 0.1
        #         if self.counter_grass > int(25 / (coef * np.linalg.norm((self.car.hull.linearVelocity)))):
        #             done = True
        #             step_reward = -50
